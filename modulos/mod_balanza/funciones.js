...
function AgregarBalanza() {
  ...
  if (mensajes.length == 0) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        console.log(
          "*********  enviando datos para add balanzas ****************",
        );
      },
...
function htmlPlu(idBalanza) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "*********  enviando datos para html balanzas ****************",
      );
    },
    success: function (response) {
...
function buscarProducto(caja, valor) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "*********  enviando datos para buscar productos ****************",
      );
    },
    success: function (response) {
      ...
      if (resultado["datos"]) {
        datosEnInput(
          resultado["datos"]["idArticulo"],
          resultado["datos"]["nombre"],
          resultado["datos"]["referencia"],
          resultado["datos"]["codBarras"],
        );
      } else {
...
function ModificarBalanza(id) {
  ...
  if (mensajes.length === 0) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        console.log(
          "*********  enviando modificar datos principales balanza ****************",
        );
      },
      success: function (response) {
...
function CrearDirectorioBalanza(idBalanza) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "*********  enviando crear directorio balanza ****************",
      );
    },
    success: function (response) {
...
function validarCamposBalanza(nombreBalanza, modeloBalanza, secciones) {
  ...
  if (!nombreBalanza || !modeloBalanza || !secciones) {
    respuesta["Texto"] =
      "Debe completar Nombre Balanza, Modelo Balanza y Secciones.";
  }
  return respuesta;
}
...
function validarGrupoDireccionIp(grupo, direccion, ip) {
  ...
  if ((grupo || direccion || ip) && (!grupo || !direccion || !ip)) {
    respuesta["Texto"] =
      "Si completa Grupo, Dirección o IP, debe completar los tres campos.";
  }
  return respuesta;
}
...
function validarFormatoIp(ip) {
  ...
  if (ip) {
    ...
    if (!ipRegex.test(ip)) {
      respuesta["Texto"] =
        "La IP debe tener el formato correcto (ej: 192.168.4.23), sin ceros a la izquierda y cada grupo de 1 a 3 dígitos.";
    }
  }
  return respuesta;
}
...
function validarNumeroDosDigitos(valor, campoNombre) {
  ...
  if (valor && !/^\d{1,2}$/.test(valor)) {
    respuesta["Texto"] =
      "El " + campoNombre + " debe ser un número de 1 o 2 dígitos.";
  }
  return respuesta;
}
...
function pluInputsModificados(idArticulo) {
  ...
  if ($teclaInput.length && $teclaInput.val() !== $teclaInput.data("original"))
    modificado = true;
  return modificado;
}
...
function guardarTodos(idBalanza) {
  ...
  $('tr[id^="plu_"]').each(function () {
    ...
    var seccion = $("#editTecla_" + idArticulo).length
      ? $("#editTecla_" + idArticulo).val()
      : null;
    datos.push({
...
function mostrarArticulosPeso(idBalanza) {
  console.log(
    "mostrarArticulosPeso: solicitando artículos de peso para balanza",
    idBalanza,
  );
  $.ajax({
    url: "tareas.php",
    ...
    error: function (xhr, status, error) {
      console.error(
        "mostrarArticulosPeso: error en la petición AJAX",
        status,
        error,
      );
    },
...
function addArticuloPeso(idArticulo, idBalanza) {
  ...
  if (plu) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        console.log(
          "addArticuloPeso: enviando petición AJAX para añadir PLU...",
        );
      },
      success: function (response) {
      ...
      error: function (xhr, status, error) {
        console.error(
          "addArticuloPeso: error en la petición AJAX",
          status,
          error,
        );
      },
...
function guardarConfigAvanzada(idBalanza) {
  ...
  respuesta = validarGrupoDireccionIp(
    grupoBalanza,
    direccionBalanza,
    ipBalanza,
  );
  ...
  if (mensajes.length === 0) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        console.log(
          "*********  enviando guardar configuración avanzada balanza ****************",
        );
      },
      success: function (response) {
        var resultado = $.parseJSON(response);
...
function volcadoCompleto(idBalanza) {
  var parametros = {
    pulsado: "volcadoCompleto",
    idBalanza: idBalanza,
    plusBalanza: JSON.stringify(plusBalanza),
  };
  console.log("Iniciando volcado completo con parámetros:", parametros);
  $.ajax({
    data: parametros,
    url: "tareas.php",
    type: "post",
    beforeSend: function () {
      console.log(
        "*********  enviando solicitud de volcado completo de la balanza ****************",
      );
    },
    success: function (response) {
      var resultado = $.parseJSON(response);
      if (resultado.error) {
        // Se muestra el error dentro de div divAlertas
        $("#divAlertas").html(
        ...
        // Si el volcado es exitoso, se muestra un mensaje de éxito
        $("#divAlertas").html(
