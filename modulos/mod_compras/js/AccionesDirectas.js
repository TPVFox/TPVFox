...
function AccionBuscarProductos(caja, event) {
  ...
  if (busqueda !== "" || idcaja === "Descripcion") {
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log(
          "******** Respuesta de FUNCION -> buscarProducto *********"
        );
        ...
        if (resultado.Nitems === 1 && idcaja != "cajaBusqueda") {
          ...
          console.log(
            " Entramos en AccionBuscarProducto->Añadir un resultado "
          );
          if (resultado["datos"][0]["estadoTabla"] == "Baja") {
            alert(
              "Este producto no se puede adjuntar ya que el estado del producto es BAJA"
            );
          } else {
            ...
            AntesAgregarFilaProducto(
              datos,
              dedonde,
              resultado["datos"][0]["fechaActualizacion"],
              resultado["datos"][0]["ultimoCoste"]
            );
          }
        } else {
