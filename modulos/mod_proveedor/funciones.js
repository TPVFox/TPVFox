  ...
  switch (pulsado) {
    ...
    case "ListadoProductos":
      ...
      window.location.href =
        "./OtrasVistas/ListadoProductosDeProveedor.php?id=" + checkID[0];
      break;
...
function abrirResumen(idProveedor, tipoDoc = "") {
  ...
  window.location.href =
    "./OtrasVistas/resumenAlbaranes.php?id=" + idProveedor + parametros;
}
...
function imprimirSeleccion(id) {
  if (
    $(".table tbody input").is(":checked") &&
    $(".table tbody input").not([(style = "display:none")])
  ) {
...
function obtenerEstadoProductoWeb(ids_productos, id_tiendaWeb) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "*********  Obteniendo Estado de productos de la web  ****************"
      );
    },
    success: function (response) {
      ...
      resultado.forEach(function (producto) {
        ...
        if (producto.estado === "1") {
          $("#idProducto_estadoWeb_" + producto.idArticulo).removeClass(
            "icono_web despublicado"
          );
          //~ console.log(producto.idArticulo);
        }
        if (producto.estado === "Error") {
          $("#idProducto_estadoWeb_" + producto.idArticulo)
            .removeClass("icono_web despublicado")
