...
function validarChecks() {
  ...
  if (checkID.length > 1 || checkID.length === 0) {
    alert(
      "¿Cuantos items tienes seleccionados? \n Sólo puedes tener uno seleccionado"
    );
    return false;
  }
  return true;
...
function borrarArticulo(idarticulo) {
  $.ajax({
    data: {
      pulsado: "Borrar_producto_tarifa_cliente",
      idcliente: cliente.idClientes,
    ...
    beforeSend: function () {
      console.log(
        "*********  Eliminando producto de tarifa del cliente **************"
      );
    },
...
function resumen(dedonde, idCliente) {
  ...
  if (dedonde === "tickets") {
    window.location.href =
      "./Resumenes/resumenTickets.php?id=" + idCliente + "&historyJS=1";
  }
  if (dedonde === "albaranes") {
    window.location.href =
      "./Resumenes/resumenAlbaranes.php?id=" + idCliente + "&historyJS=1";
  }
}
function imprimirResumen(dedonde, id, fechaInicial, fechaFinal) {
  var parametros = {
    pulsado: "imprimirResumen",
    dedonde: dedonde,
    idCliente: id,
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (resultado.error) {
        console.log(resultado.error["dato"]);
        titulo = "Error";
        abrirModal(titulo, resultado.error["dato"]);
      } else {
