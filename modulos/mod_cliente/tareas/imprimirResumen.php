<?php 
// Imprimimos Resumen de Tickets o de Albaranes.
...
if($_POST['fechaInicial'] !='' & $_POST['fechaFinal']!=''){
    ...
    if ($_POST['dedonde'] == 'ticket'){
        $arrayNums=$Cliente->ticketClienteFechas($idCliente, $fechaInicial, $fechaFinal);
        $documento = 'Numticket';
        $titulo = 'Facturas simplificadas (Tickets)';
    } else {
        $arrayNums=$Cliente->albaranClienteFechas($idCliente, $fechaInicial, $fechaFinal);
        $documento = 'Numalbcli';
        $titulo = 'Albaranes del cliente';
    }
    ...
    if(isset($arrayNums['error'])){
        ...
        $totalbases=0;
            foreach($arrayNums['resumenBases'] as $bases){
                ...
                if ($_POST['dedonde'] == 'ticket'){
                    $numDocumento = $bases['idTienda'].'-'.$bases['idUsuario'].'-'.$bases[$documento];
                } else {
                    $numDocumento = $bases[$documento];
                }
                $html.= '<tr>
                    <td WIDTH="50%"><font size="8">'.$bases['fecha'].'</font></td>
                    <td><font size="8">'.$numDocumento.'</font></td>
        ...
        $html.='<h3>Desglose de productos de las '.$titulo.'</h3>
