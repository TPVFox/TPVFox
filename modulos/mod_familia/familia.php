...
<html>
    <head>
        ...
        if ($ClasePermisos->getAccion("VerFamiliaWeb") == 1) {
                    ...
                    if ( isset ($t['Datos']['item'])){
                        ...
                        foreach ($todasFamiliasWeb as $key => $familiaWeb){
                            ...
                            if (!isset($existe['datos'])){
                                // Si no existe relacion de esa familia web la eliminamos del array y mostramos una advertencia.
                                $errores[] =  $Cfamilias->montarAdvertencia('warning',
                                                'Existe la familia:<b>'.$familiaWeb['nombre'].'</b><br/>'
                                                .'En la web, pero no tiene relacion con tpv, por eso no en el select de padres de la web'
                                                );
                        ...
                        $combopadresWeb = $Cfamilias->htmlComboFamilias($todasFamiliasWeb, $id_padre_web,'virtuemart_category_id');
                            ...
                            $errores[] = $Cfamilias->montarAdvertencia('warning',
                                                'No se pudo obtener ninguna familia web.'
                                                );
                    }
