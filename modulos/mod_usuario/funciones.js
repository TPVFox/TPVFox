...
function eliminarConfiguracionModulo(idUsuario, modulo) {
  var mensaje = confirm(
    "¿Estás seguro que quieres eliminar la configuración del usuario?"
  );
  if (mensaje) {
    var parametros = {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        console.log(
          "*********  Eliminar configuración del modulo   **************"
        );
      },
      success: function (response) {
...
function copiarPermisosUsuario() {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "********* Copiar los permisos de un usuario a otro  **************"
      );
    },
    success: function (response) {
...
function confirmarCambiarAnoUsuarios() {
  ...
  if (nuevaContrasena.trim() === "") {
    alert(
      "Por favor, introduce una nueva contraseña para los usuarios seleccionados."
    );
    return;
  }
