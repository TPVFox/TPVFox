...
function modificarFamiliaWeb(idFamilia = "", idTienda = "") {
  ...
  var mensaje = confirm(
    "¿Estás seguro que quieres AÑADIR / MODIFICAR la tienda en la web?"
  );
  if (mensaje) {
    if ($("#nombreFamilia").val() == "") {
      ...
      $.ajax({
        ...
        beforeSend: function () {
          console.log(
            "********* Envio los datos para modificar la familia en la web  **************"
          );
        },
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (resultado.familiasSubidas.length > 0) {
        alert(
          "Familias subidas : " + JSON.stringify(resultado.familiasSubidas)
        );
      }
      if (resultado.familiasNoSubidas.length > 0) {
        alert(
          "Familias NO subidas : " + JSON.stringify(resultado.familiasNoSubidas)
        );
      }
    },
  });
