...
function modificarProductoWeb(idProducto = "", idTiendaWeb = "") {
  ...
  var mensaje = confirm(
    "¿Estás seguro que quieres AÑADIR / MODIFICAR el producto en la web?"
  );
  var iva = $("#ivasWeb").val();
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "********* Envio los datos para modificar el producto en la web  **************"
      );
    },
...
function ModalNotificacion(numLinea) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "********* Envio los datos para mostrar el modal de notificaciones  **************"
      );
    },
    success: function (response) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (resultado.errorModificacion) {
        alert(
          "No se modificó la notificación  error de SQL: " +
            resultado.errorModificacion
        );
      }
      if (resultado.mail == 1) {
...
function subirProductosWeb(idTiendaWeb) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      console.log(
        "*********  Subir conjunto de productos a la web **************"
      );
    },
    success: function (response) {
      ...
      if (resultado.productoEnWeb.length > 0) {
        alert(
          "Producto que YA ESTABAN y NO se subieron: " +
            JSON.stringify(resultado.productoEnWeb)
        );
      }
      if (resultado.errores) {
        alert(
          "¡¡¡ ERRORES !!! Hubo los siguientes errores" +
            JSON.stringify(resultado.errores)
        );
      }
      if (resultado.contadorProductos > 0) {
        alert(
          "Se han subido a la web :" +
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (resultado["productos"]["Datos"]["item"].length > 0) {
        ...
        console.log(productos);
        console.log(
          "Productos en vuelta" + resultado["productos"]["Datos"]["item"].length
        );
