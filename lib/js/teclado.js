...
function controlEventos(event){
	...
	if (typeof padre_caja === 'object'){
        ...
        if ( event.keyCode !== undefined){
            ...
            var accion = caja.darAccion(event.keyCode);
            console.log(accion);
            if (caja.darAccion(event.keyCode) !== 'error'){
    ...
    if (event.type === 'blur' || event.type==='click'){
		...
		if (typeof caja.darParametro('pulsado_intro') !== 'undefined'){
			if (caja.darParametro('pulsado_intro') === 'Si'){
				// Si pulso intro, que lo controlamos con un parametro en la caja, no repetimos la accion.
				// LLegamos aquí porque perdería el focus por lo que no hacemos accion.
